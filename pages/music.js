...
export default function MusicPage() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const { downloadURL, storagePath } = await uploadTrackFile(user.uid, file, (val) => setProgress(Math.max(1, val)), {
        title,
        artist,
        album: form.album,
        genre: form.genre,
  ...
  return <AppShell title="Music | NearBeat"><div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 lg:px-8 lg:py-8"><PageHeader eyebrow="Music" title="Shared music library" description="Upload and discover real tracks from Firebase." />
    <section className="mt-6 rounded-3xl border border-white/10 bg-white/[0.05] p-4">
      ...
      <label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-black text-slate-950"><Upload size={16} /> {busy ? `${uploadState === 'saving' ? 'Saving post' : `Uploading ${progress}%`}` : 'Upload track'}<input type="file" accept="audio/*" hidden onChange={handleUpload} disabled={busy} /></label>
