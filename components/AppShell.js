      ...
      <main className="min-h-screen bg-[#080a10] text-slate-100">
        <div className="mx-auto grid min-h-screen w-full max-w-7xl lg:grid-cols-[248px_1fr]">
          ...
          <section className="min-w-0 pb-24 lg:pb-0">
            <header className="sticky top-0 z-30 border-b border-white/10 bg-[#080a10]/82 px-4 py-3 backdrop-blur-xl sm:px-6 lg:hidden"><div className="flex items-center justify-between"><Link href="/feed" className="font-black tracking-tight">{online ? <span className='flex items-center gap-2'><span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-400 text-slate-950"><Radio size={19} /></span>NearBeat</span> : 'Connection…'}</Link><Link href="/search" className="rounded-full bg-white/10 p-2"><Compass size={19} /></Link></div></header>
                  ...
                  return (
                    <Link
                      ...
                      className={`flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold transition ${
                        active ? 'bg-white text-slate-950' : 'bg-white/5 text-slate-300'
                      }`}
                    >
                      <Icon size={16} />
