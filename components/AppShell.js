...
export default function AppShell({ children, title = 'NearBeat' }) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('online', onState); window.removeEventListener('offline', onState); };
  }, []);
      ...
      <main className="min-h-screen bg-[#080a10] text-slate-100">
        <div className="mx-auto grid min-h-screen w-full max-w-7xl lg:grid-cols-[248px_1fr]">
          <aside className="sticky top-0 hidden h-screen border-r border-white/8 bg-black/20 px-4 py-5 backdrop-blur-xl lg:block">
            ...
            <nav className="mt-8 space-y-1">{navItems.map((item) => { const Icon = item.icon; const active = router.pathname === item.href; return <motion.div key={item.href} whileHover={{ x: 3 }}><Link href={item.href} className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition ${active ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-white/8 hover:text-white'}`}><Icon size={19} />{item.label}</Link></motion.div>; })}</nav>
            <div className="absolute bottom-5 left-4 right-4 rounded-3xl border border-white/8 bg-white/[0.04] p-4"><div className="flex items-center gap-3"><UserCircle className="text-emerald-300" size={24} /><div><p className="text-sm font-bold">{profile?.displayName || 'NearBeat User'}</p><p className="text-xs text-slate-400">{profile?.systemId || ''}</p></div></div><button onClick={logout} className="mt-3 inline-flex items-center gap-2 text-xs text-slate-300"><LogOut size={14} />Sign out</button></div>
          </aside>
          <section className="min-w-0 pb-24 lg:pb-0">
            <header className="sticky top-0 z-30 border-b border-white/8 bg-[#080a10]/82 px-4 py-3 backdrop-blur-xl sm:px-6 lg:hidden"><div className="flex items-center justify-between"><Link href="/feed" className="font-black tracking-tight">{online ? <span className='flex items-center gap-2'><span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-400 text-slate-950"><Radio size={19} /></span>NearBeat</span> : 'Connection…'}</Link><Link href="/search" className="rounded-full bg-white/8 p-2"><Compass size={19} /></Link></div></header>
            <AnimatePresence mode="wait"><motion.div key={router.asPath} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}>{children}</motion.div></AnimatePresence>
          </section>
        </div>
      </main>
    </>
