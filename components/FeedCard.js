...
export default function FeedCard({ listener, index = 0 }) {
  ...
  return (
    <motion.article
      ...
      className="glass-card group relative overflow-hidden rounded-[2rem] p-4 transition duration-300 hover:border-pulse/40 hover:shadow-glow"
    >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-white">{listener.username}</p>
                <div className="mt-1 flex items-center gap-1.5 text-xs text-pulse">
                  <Signal size={13} />
                  <span>live now</span>
                  <span className="flex gap-0.5 pl-1">
                    {[0, 1, 2].map((bar) => (
                      <span
                        key={bar}
                        className={`h-1.5 w-1.5 rounded-full ${pulse === bar ? 'bg-pulse' : 'bg-pulse/30'}`}
                      />
                    ))}
                  </span>
                </div>
          ...
          <div className="mt-5">
            <h3 className="truncate text-2xl font-black tracking-tight text-white">{listener.songTitle}</h3>
            <p className="mt-1 truncate text-sm text-slate-300">{listener.artist}</p>
          </div>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2 text-sm text-slate-300">
            <span className="flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1.5">
              <MapPin size={15} className="text-neonBlue" />
              {listener.distance}
            </span>
            <span className="flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1.5">
              <MessageCircle size={15} className="text-neonPink" />
              {listener.note}
            </span>
